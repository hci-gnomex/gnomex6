...
sampeIdRegex="TL-[0-9]{2}-([A-Za-z0-9]{6}).*"
...
while read fileName; do
        ...
        if [[ $fileName =~ $regex ]] ; then
                ...
                if [ ! -z "$matchFastqFile" ]; then  #If var is not empty
                        ...
                        joinedFastqStr="$downloadPath"$(basename -- $match) #$(ls "$downloadPath""$matchFastqID"*"$matchNucType"*"gz")"
                        ...
                        tar xvfz $joinedFastqStr -C "$downloadPath""/""$matchNucType"
...
#echo  $tempusRequestList
#echo This is the sample regex $sampeIdRegex
#bash register_files.sh -doNotSendMail -onlyExperiment
#bash linkData.sh -dataSource 10R -regex $sampeIdRegex -debug  -requests $tempusRequestList
