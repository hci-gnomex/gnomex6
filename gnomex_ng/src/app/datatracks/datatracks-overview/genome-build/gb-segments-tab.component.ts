...
@Component({
    ...
    template: `
        <span>
            <span> Segments: {{this.rowData.length }} </span> 
    ...
    removeSegments(){
        ...
        for(let i = 0; i < selectedRows.length; i++){
            tmpRowData = tmpRowData.filter( rowObj =>{
                return rowObj.name !== selectedRows[i].name ||
                       rowObj.length !== selectedRows[i].length ||
                       rowObj.sortOrder !== selectedRows[i].sortOrder} );
