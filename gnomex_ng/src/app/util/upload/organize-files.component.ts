...
export class OrganizeFilesComponent implements OnInit, AfterViewInit{
  ...
  private _nodeClass = (node: TreeNode): string => {
    if (!node || !node.data) { return ''; }  // ← this is the actual fix
  ...
  uploadTreeOnSelect(event:any){
    ...
    const otherTreeFocusedNode =  this.organizeTree.treeModel.getFocusedNode();
  ...
  organizeTreeOnSelect(event:any){
    if (!event || !event.node) { return; }  // ← guard against null node
    const otherTreeFocusedNode =  this.uploadTree.treeModel.getFocusedNode();
    if(otherTreeFocusedNode){
      otherTreeFocusedNode.setIsActive(false);
    }
