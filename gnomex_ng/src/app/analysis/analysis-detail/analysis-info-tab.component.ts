...
export class AnalysisInfoTabComponent implements OnInit, OnDestroy, OnChanges {
  ...
  constructor(private route: ActivatedRoute,
    ...
    this.form = this.formBuilder.group({
      ...
      submitterName: [{value: ""}],
      submitDate: [{value: ""}],
      codeVisibility: [{value: ""}, Validators.required],
      idInstitution: [{value: "", disabled: true}],
      collaboratorsJSONString: [],
      genomeBuildToAdd: [{value: null, disabled: true}],
      privacyExpirationDate: [{value: ""}],
    });
  }
  ...
  ngOnInit() {
    ...
    this.form.controls["codeVisibility"].valueChanges.subscribe(() => {
      if (this.form.controls["codeVisibility"].value === "INST" && this.isEditMode && this.canUpdate) {
  ...
  /** Applies to: name, idAnalysisType, idAnalysisProtocol, idOrganism,
   *  genomeBuildsJSONString, codeVisibility, collaboratorsJSONString,
   *  genomeBuildToAdd, privacyExpirationDate */
  public get isReadonly(): boolean {
    return !this.isEditMode || !this.canUpdate;
  }

  /** Applies to: idAppUser — additionally requires canWriteAnyObject */
  public get isOwnerReadonly(): boolean {
    return !this.isEditMode || !this.canUpdate || !this.canWriteAnyObject;
  }

  /** Applies to: idInstitution — only editable when visibility is INST */
  public get isInstitutionReadonly(): boolean {
    return !this.isEditMode || !this.canUpdate
      || this.form.controls["codeVisibility"].value !== "INST";
  }
  ...
  public openCollaboratorsWindow(): void {
    if (this.isReadonly || this.form.controls["codeVisibility"].value === "PUBLIC") {
      return;
    }
    if (!this.lab) {
      let params: HttpParams = new HttpParams()
  ...
  updateForm(): void {
    // custom-combo-box controls are managed via enable()/disable() as before.
    // Native inputs and radio groups use readonly/appReadonly in the template instead.
    ...
    if (this.canUpdate && this.isEditMode) {
      ...
      if (this.form.controls["codeVisibility"].value === "INST") {
        this.form.controls["idInstitution"].enable();
      }
