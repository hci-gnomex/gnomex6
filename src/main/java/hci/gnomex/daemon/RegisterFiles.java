...
public class RegisterFiles extends TimerTask {
  ...
  Map<String, List<AnalysisFileInfo>> analysisFileMap =
      new HashMap<String, List<AnalysisFileInfo>>();
  ...
  public void run() {
    ...
    errorMessageString +=
        " on " + new SimpleDateFormat("MM-dd-yyyy_HH:mm:ss").format(runDate.getTime()) + "\n";
    ...
    try {
      ...
      String msg =
          "Could not remove links or register experiment or analysis files (error at "
  ...
  private void initialize() throws Exception {
    ...
    baseFlowCellDir =
        PropertyDictionaryHelper.getInstance(sess)
            .getDirectory(serverName, null, PropertyDictionaryHelper.PROPERTY_FLOWCELL_DIRECTORY);
    baseAnalysisDir =
        PropertyDictionaryHelper.getInstance(sess)
    ...
    gnomexSupportEmail =
        ph.getQualifiedProperty(PropertyDictionary.GNOMEX_SUPPORT_EMAIL, serverName);
    if (gnomexSupportEmail == null) {
      gnomexSupportEmail =
          ph.getQualifiedProperty(PropertyDictionary.CONTACT_EMAIL_SOFTWARE_TESTER, serverName);
    }
  ...
  private void destroyLinks() throws Exception {
    dataTrackFileServerWebContext =
        PropertyDictionaryHelper.getInstance(sess)
            .getProperty(PropertyDictionary.DATATRACK_FILESERVER_WEB_CONTEXT);
  ...
  private void registerExperimentFiles() throws Exception {
    ...
    if (!justOne && asOfDate != null) {
      buf.append(
          " WHERE r.createDate >= '"
              + new SimpleDateFormat("yyyy-MM-dd").format(asOfDate.getTime())
              + "'");
    }
    ...
    for (Iterator i = results.iterator(); i.hasNext(); ) {
      ...
      if (request.getFiles() != null) {
        ...
        String directoryName =
            baseExperimentDir
                + Request.getCreateYear(request.getCreateDate())
                + "/"
        ...
        for (Iterator i2 = request.getFiles().iterator(); i2.hasNext(); ) {
          ...
          if (fd == null && newFilePath == null) {
            ...
            if (ef.getFileSize() == null
                || !ef.getFileSize().equals(BigDecimal.valueOf(fd.getFileSize()))) {
  ...
  private java.sql.Date getEffectiveRequestFileCreateDate(
      FileDescriptor fd, java.util.Date requestCreateDate) {
  ...
  private void registerAnalysisFiles() throws Exception {
        ...
        if (fd == null) {
          ...
          if (sendMail || testingSendMail) {
            ...
            if (af.getComments() != null && !af.getComments().trim().equals("")) {
              // Get an email address for the file
              String emailAddress = "";
              if (af.getAnalysis().getAppUser() != null
                  && af.getAnalysis().getAppUser().getEmail() != null) {
          ...
          if (af.getFileSize() == null
              || !af.getFileSize().equals(BigDecimal.valueOf(fd.getFileSize()))) {
    ...
    try {
      ...
      String msg =
          "Unable to send warning email notifying user that analysis files have been deleted:  "
              + e.toString()
              + "\n\t";
  ...
  private java.sql.Date getEffectiveAnalysisFileCreateDate(
      FileDescriptor fd, java.util.Date analysisCreateDate) {
    java.sql.Date createDate = new java.sql.Date(runDate.getTime().getTime());
  ...
  private void sendNotifyEmails(String fromAddress)
    ...
    for (Iterator i = this.emailAnalysisMap.keySet().iterator(); i.hasNext(); ) {
      ...
      body.append(
          "The following analysis files no longer exist on the file system and have been removed from the database.  Please remove the associated data tracks.");
  ...
  private Map hashFiles(
    ...
    String baseExperimentDir =
            PropertyDictionaryHelper.getInstance(sess)
                    .getDirectory(
                            serverName, idCoreFacility, PropertyDictionaryHelper.PROPERTY_EXPERIMENT_DIRECTORY);
    String directoryName =
            baseExperimentDir + Request.getCreateYear(createDate) + "/" + baseRequestNumber;
    directoryName.replace("\\", "/");
    ...
    if (fileList != null) {
      for (int x = 0; x < fileList.length; x++) {
        ...
        if (f1.isFile()) { // && !Util.isSymlink(f1)) {
          FileDescriptor fd =
                  new FileDescriptor(
                          requestNumber, fileList[x], f1, baseRequestNumber + "/" + fileList[x]);
    ...
    Set folders =
        GetRequestDownloadList.getRequestDownloadFolders(
            baseExperimentDir,
            baseRequestNumber,
            Request.getCreateYear(createDate),
            codeRequestCategory);
    for (Iterator i1 = folders.iterator(); i1.hasNext(); ) {
      ...
      UploadDownloadHelper.getFileNamesToDownload(
          ...
          serverName,
          null,
          Request.getKey(requestNumber, createDate, folderName, idCoreFacility),
          requestNumbers,
          requestMap,
          directoryMap,
          flowCellDirFlag);
  ...
  private HashMap hashFiles(Analysis analysis) throws Exception {
    ...
    GetExpandedAnalysisFileList.getFileNamesToDownload(
        baseAnalysisDir, analysis.getKey(), analysisNumbers, analysisMap, directoryMap, false);
  ...
  private String recurseDirectoriesForFile(
      String fileToLookFor, BigDecimal sizeOfFile, Request r, LinkedList<String> dirs) {
    if (dirs.isEmpty()) {
      return null;
    }
  ...
  private void deleteExpFileAndNotify(Session sess, ExperimentFile ef) {
    List sampleExperimentFiles =
        sess.createQuery(
                "Select sef from SampleExperimentFile sef where idExpFileRead1 = "
                    + ef.getIdExperimentFile()
                    + " OR idExpFileRead2 = "
                    + ef.getIdExperimentFile())
            .list();
    ...
    printDebugStatement(
        "SIZE OF SAMPLE_EXPERIMENT_FILES_LIST::::::::: " + sampleExperimentFiles.size());
    ...
    for (Iterator i = sampleExperimentFiles.iterator(); i.hasNext(); ) {
      ...
      if (sef.getIdExpFileRead1() != null
          && ef.getIdExperimentFile().equals(sef.getIdExpFileRead1())) {
        sef.setIdExpFileRead1(null);
      } else if (sef.getIdExpFileRead2() != null
          && ef.getIdExperimentFile().equals(sef.getIdExpFileRead2())) {
    ...
    if (!listOfSampleNames.equals("") && sendMail) {
      ...
      if (!MailUtil.isValidEmail(toAddress)) {
        System.err.println(
            "Unable to notify of unlinking of sample experiment files due to invalid email address:  "
                + toAddress);
        return;
      }
      ...
      try {
        ...
        System.err.println(
            "WARNING: Unable to send email notifying of deletion of Sample Experiment Files. Trying to send to: "
                + toAddress
                + e.toString());
      }
    }
...
//          System.out.println("Found a loop!");
  ...
  public static class AnalysisFileInfo {
    ...
    public AnalysisFileInfo(
        String analysisNumber, String analysisFileName, DataTrack dataTrack, String comment) {
      super();
