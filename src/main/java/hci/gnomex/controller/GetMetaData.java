...
public class GetMetaData extends GNomExCommand implements Serializable {

  private static Logger LOG = Logger.getLogger(GetMetaData.class);
  ...
  public void loadCommand(HttpServletWrappedRequest request, HttpSession session) {
    ...
    if (request.getParameter("showUploads") != null && !request.getParameter("showUploads").equals("")) {
      ...
      System.out.println ("[GetMetaData] showUploads: " + showUploads);
    }
...
//          System.out.println ("[GetMetaData] xmlResult:\n" + xmlResult);
          ...
          this.xmlResult = "<SUCCESS>" + theMetaFile + "</SUCCESS>";
      ...
      this.errorDetails = Util.GNLOG(LOG,"An exception has occurred in GetMetaData ", e);
    ...
    String dinfo = "GetMetaData (" + this.getUsername() + " - " + reqNumber + "), ";
  ...
  private static void appendWorkflowStatusNodes(Request request, Element requestNode, Session sess) throws Exception {
    ...
    for (String workflowStep : workflowStepHash.keySet()) {
      ...
      stepNode.setAttribute("stepName", workflow.get(Integer.valueOf(workflowStep).intValue()-1).toString());
