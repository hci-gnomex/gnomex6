...
public class Util {
  ...
  public static String listToString(
      List list, String delimiter, String prefix, String postfix, int maxItems) {
    if (delimiter == null) {
      delimiter = ",";
    }
    if (prefix == null) {
  ...
  public static Integer retrieveRequestIntegerParameter(
      HttpServletRequest request, String parameterName, Command command) {
  ...
  public static Boolean retrieveRequestBooleanParameter(
      HttpServletRequest request, String parameterName, Command command) {
    return retrieveRequestBooleanParameter(request, parameterName, command, null);
  }

  public static Boolean retrieveRequestBooleanParameter(
      HttpServletRequest request, String parameterName, Command command, Boolean defaultValue) {
    String parameter = request.getParameter(parameterName);
  ...
  public static String retrieveRequestStringParameter(
  ...
  public static void sendErrorReport(
      ...
      String softwareTestEmail,
      String fromAddress,
      String userName,
      String errorMessage,
      StringBuilder requestDump) {
    try {
      ...
      // If the error occurred on a developer workstation, get their email address and use it
      // instead
      ...
      MailUtilHelper helper =
          new MailUtilHelper(
              ...
              null,
              fromAddress,
              "GNomEx Runtime Error [Server: " + localMachine.getHostName() + "]",
              errorMessageString,
              null,
              false,
    ...
    if (serverName != null && !serverName.equals("")) {
      ...
      String machinelist =
          PropertyDictionaryHelper.getInstance(sess)
              .getProperty(PropertyDictionaryHelper.PROPERTY_RUNTIME_ERROR_SERVER_LIST);
  ...
  public static void createTransferLogFile(
      String taskFileDir, String uuidStr, StringBuilder filesToTransfer, String transferLogPath) {
    ...
    while (true) {
      ...
      if (!taskFile.exists()) {
        ...
        try {
          ...
          if (!success) {
            System.out.println(
                "[createTransferLogFile] Error: unable to create fdtDownloadTransferLog file. "
                    + taskFileName);
            return;
          }
          break;
    ...
    try {
      ...
      System.out.println(
          "[createTransferLogFile] IOException: file " + taskFileName + " " + e.getMessage());
      return;
    }
  }
      ...
      if (iconLookupMap.containsKey(node.getName())) {
        ...
        if (node.getName().equals("FileDescriptor")) {
          if (node.getAttributeValue("type") != null
              && node.getAttributeValue("type").equals("dir")) {
            if (node.getAttributeValue("isEmpty") != null
                && node.getAttributeValue("isEmpty").equals("Y")) {
              icon = "assets/folder_disable.png";
            } else {
              icon = "assets/folder.png";
            }
          } else if (node.getAttributeValue("isSupportedDataTrack") != null
              && node.getAttributeValue("isSupportedDataTrack").equals("Y")) {
  ...
  private static void setFileDescriptorDisplayRecursively(Element node) {
    if (node.getName().equals("FileDescriptor")) {
      ...
      if (isParameterNonEmpty(node.getAttributeValue("viewURL"))) {
        color = "blue";
        if (node.getAttributeValue("PROTECTED") != null
            && node.getAttributeValue("PROTECTED").equals("Y")) {
          color = "purple";
        }
      } else {
        if (node.getAttributeValue("PROTECTED") != null
  ...
  private static ObjectNode convertXMLElementToObjectNode(
      final ObjectMapper mapper, final Element element, ObjectNode node) {
  ...
  public static void preserveXMLNodeName(Element experimentItem) {
    ...
    for (int i = 0; i < children.size(); i++) {
      Element childNode = children.get(i);
  ...
  public static String getAppUserDisplayName(AppUser userToPrint, UserPreferences userPreferences) {
    return userPreferences.getFormatNamesFirstLast()
        ? userToPrint.getFirstLastDisplayName()
        : userToPrint.getDisplayName();
  }

  public static String formatUserDisplayName(
      String firstName, String lastName, UserPreferences userPreferences) {
  ...
  public static String formatLabDisplayName(
      String firstName, String lastName, UserPreferences userPreferences) {
    StringBuilder builder = new StringBuilder();
    if (userPreferences.getFormatNamesFirstLast()) {
      if (firstName != null && firstName.length() > 0) {
  ...
  public static void buildAndSendUploadFileServletURL(
      HttpServletRequest request,
      HttpServletResponse response,
      Session session,
      String urlServletName,
      String uploadServletName,
    ...
    // String portNumber =
    // PropertyDictionaryHelper.getInstance(session).getQualifiedProperty(PropertyDictionary.HTTP_PORT, request.getServerName());
    String portNumber = "" + request.getServerPort();
    String stringURL = request.getRequestURL().toString();
    System.out.println("[buildAndSendUploadFileServletURL] (1) portnumber: " + portNumber);

    // example https://b2b.hci.utah.edu:445
    if (portNumber == null || portNumber.length() == 0) {
      String[] pieces = stringURL.split(":");
      if (pieces.length == 3) {
        portNumber = pieces[1];
        System.out.println("[buildAndSendUploadFileServletURL] (2) portNumber: " + portNumber);
      }
    }
    if ((portNumber == null || portNumber.equals(""))) { // || requireSecureMode) && !isLocalHost) {
      portNumber = "";
    } else {
    ...
    System.out.println(
        "[buildAndSendUploadFileServletURL] (3) portnumber: "
            + portNumber
            + " request url "
            + request.getRequestURL());
    String URL =
        "http"
            + (isLocalHost || !requireSecureMode ? "://" : "s://")
