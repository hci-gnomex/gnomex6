 ...
 * <p>
 * This class follows the singleton idiom for Java, in that classes may not directly instantiate a
 * DictionaryManager, but may retrieve an instance that is instantiated only once for the project.
 * This singleton maintains a cache of the loaded dictionaries and their values, and provides the
 * interface through which the dictionaries can be interacted with.
 * <p>
 ...
 * <br>
 * <li>Caching behavior to initially load the dictionary entries, using Hibernate, and retrieve
 * entries from cache for subsequent calls</li>
 * <br>
 * <li>Methods to return the values of an individual dictionary, or all dictionaries as XML
 * data</li>
 * <br>
 * <li>The ability to process inserts, updates, and deletes and automatically persist changes to
 * any
 * dictionary class extending DictionaryEntry, as well as updating the cached values for the
 * modified dictionary entry</li>
 * <br>
 * <li>Methods which can provide the functionality for the loadCommand and execute methods of an
 * instance of the {@link hci.dictionary.utility.DictionaryCommand} class, that wishes to make use
 * of the DictionaryManager. These methods wrap all logic needed to retrieve the appropriate
 * parameters from the HttpServletRequest and execute the desired action (specified by {@link
 * hci.dictionary.utility.DictionaryActions} ).
 * </ul>
 *
...
public final class DictionaryManager implements DictionaryActions, Serializable {
	 ...
	 *
	 * @return The singleton DictionaryManager, which can then be used to interact with the cached
	 * dictionaries
	 */
	...
	/**
	 * Return the {@link InputStream} for the dictionaryXmlFile. If this resource can be loaded as
	 * File, then it is. Otherwise it is loaded from the classpath.
	 *
	...
	/**
	 * Loads the DictionaryEntry values for all dictionaries registered with the DictionaryManager.
	 * Call RegisterDictionaries before executing this method.
	 *
	 * @param sess The Hibernate session to which the DictionarEntry classes are mapped and which
	 *             should be used to load the dictionary entries
	 */
	 ...
	 * <p>
	 * &lt;DICTIONARIES&gt;<br> &nbsp;&nbsp;&lt;Dictionary className="hci.project.model.foo"
	 * displayName="Foo Dictionary"&gt;<br> &nbsp;&nbsp;&nbsp;&nbsp;&lt;DictionaryEntry display="Entry
	 * Numero Uno" value="1" /&gt;<br> &nbsp;&nbsp;&nbsp;&nbsp;...<br> &nbsp;&nbsp;&lt;/Dictionary&gt;<br>
	 * &lt;/DICTIONARIES&gt;
	 *
	 * @param className A String representation of the name of the dictionary Class (ex.
	...
	/**
	 * Loads the specified command object with the parameters required to execute a given
	 * DictionaryAction, from the http request.
	 * <p>
	 * A valid action (specified by {@link hci.dictionary.utility.DictionaryActions}) must be included
	 * as a parameter named 'action' in the http request. If the action parameter exists and has an
	 * appropriate value, this method will load the necessary parameters from the request into the
	 * {@link hci.dictionary.utility.DictionaryCommand}, in order to later process the command when
	 ...
	 * </ul>
	 *
	 * @param command A {@link hci.dictionary.utility.DictionaryCommand} in which to load required
	 ...
	 * <p>
	 * This will look at the action property set in the DictionaryCommand, and will perform the
	 * required actions.
	 * <p>
	 * Valid actions are: <br>
	 * <ul>
	 ...
	 * </ul>
	 *
	 * @param command A {@link hci.dictionary.utility.DictionaryCommand} object, which has been
	 *                pre-loaded by the DictionaryManager's loadCommand method
	 * @param sess    A Hibernate Session that can be used to persist dictionary entries
	 */
	...
	/**
	 * Returns the display for a given dictionary class and value. If the value is not found then an
	...
	/**
	 * This Dictionary class is a private inner class of DictionaryManager, so that it is not visible
	 * to developers using the HCIDictionary.jar, since there might be some confusion about which
	 * class to extend (Dictionary or DictionaryEntry).
	 *
	 * @author sharoldsen
	 */
	private class Dictionary extends HibernateDetailObject {
		...
		public void setClassSqlMetadata(HashMap<String, SqlMetadata> classSqlMetadata) {
			this.classSqlMetadata = classSqlMetadata;
		}
		 ...
		 *
		 * @param outputStyle The style in which to forat the date, as specified in DetailObject
		...
		/**
		 * Returns this dictionary object as xml using reflection. Unlike the standard toXMLString()
		 * method in HibernateDetailObject, this does not include nodes for collections, and always uses
		 * the base class (Dictionary or DictionaryEntry, rather than a subclass) as the root node
		 *
			...
			if (this.filters != null) {
				...
				while (i.hasNext()) {
					...
					if (!f.filterClass.equals("YN")) {
						...
						field.setAttribute(
								"caption",
								parseFilterDisplay((dict != null) ? dict.getDisplayName() : this.parseDisplay(f.getFilterField()))
						);
				...
				try {
					...
					for (int i = 0; i < methods.length; i++) {
						...
						if (m.getName().indexOf("get") != -1 && m.getParameterTypes().length == 0) {
							...
							// excluded
							if ((((DictionaryEntry) o).getExcludedMethodsMap() == null
									  || ((DictionaryEntry) o).getExcludedMethodsMap().get(m.getName()) == null)
									&& !m.getName().equals("getDatakey")
									&& !m.getName().equals("getCanWrite")
									&& !m.getName().equals("getCanRead")
									&& !m.getName().equals("getCanUpdate")
									&& !m.getName().equals("getCanDelete")) {
	...
	/**
	 * This Dictionary class is a private inner class of DictionaryManager, so that it is not visible
	 * to developers using the HCIDictionary.jar. There should be no reason that it would need to be
	 * used outside the DictionaryManager and not having it visible should just make things simpler
	...
	/**
	 * This SqlMetadata class is to get the SQL metaData information of the the dictionary besides its
	 * java class metaData information
	 *
	 * @author sharoldsen
	 */
