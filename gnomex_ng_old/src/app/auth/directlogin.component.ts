...
export class DirectLoginComponent implements OnInit {
    ...
    login() {
        ...
        if (this.gnomexService.useduo) {
            if (("" + this._loginForm.value.username).match(/^[uU]\d{7,8}$/)) {
                this._authenticationService.getDuoSignRequest(this._loginForm.value.username).subscribe((r: any) => {
                    this.sig_request = r.sig_request;
                    ...
                    Duo.init({
                        iframe: "duo_iframe",
                        host: r.host || this.gnomexService.duo_host,
                        sig_request: this.sig_request,
                        submit_callback: this.twoFactorVerify.bind(this),
                    });
                }, (err: any) => {
                    this._errorMsg = "Unable to start Duo authentication.";
                });

                return; // important: stop the rest of login flow until Duo init finishes
            }
        }
    ...
    twoFactorVerify(response: any) {
        ...
        const sigResponse = response.elements.sig_response.value;

        this._authenticationService.verifyDuo(sigResponse).subscribe((r: any) => {
            console.log(r);
            if (r && r.ok) {
                this._authenticationService.requestAccessToken(true);
            } else {
