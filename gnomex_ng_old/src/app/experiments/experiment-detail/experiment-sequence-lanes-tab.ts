...
export class ExperimentSequenceLanesTab implements OnInit, OnChanges {
    ...
    ngOnInit(): void {
        ...
        let sequencingProtocolList: any[] = this.dictionaryService.getEntries(DictionaryService.NUMBER_SEQUENCING_CYCLES_ALLOWED)
            .filter(seqProtocol => {
                return seqProtocol.value
                    && seqProtocol.codeRequestCategory === this.experiment.requestCategory.codeRequestCategory
                    && seqProtocol.isActive === 'Y';
            });
        sequencingProtocolList.unshift("");
...
// removed per Brian// {headerName: "Last Cycle Status", headerTooltip: "Last Cycle Status", field: "lastCycleStatus", cellRendererFramework: SelectRenderer,
// removed per Brian//     cellEditorFramework: SelectEditor, selectOptions: workflowStatusList, selectOptionsDisplayField: "display",
// removed per Brian//     selectOptionsValueField: "value", editable: this.canEdit && this.editMode, valueSetter: this.valueChanging},
