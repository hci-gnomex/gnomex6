...
@Component({
    template: `
		<div [matTooltip]="this.errorMessage"
			...
			<div class="t full-width full-height fix-table">
				<div class="tr">
					<div class="td cell-text-container ellipsis {{ this.showInactiveSuffix ? ' inactive' : '' }}">
						{{ showInactiveSuffix && display ? display + " (inactive)" : display }}
					</div>
				</div>
			</div>
		</div>
    ...
    defaultOption: any;
    suppressInactiveSuffix: boolean = false;

    public showInactiveSuffix = false;
    ...
    agInit2(params: any): void {
        ...
        if (this.params && this.params.column && this.params.column.colDef) {
            ...
            this.suppressInactiveSuffix = !!this.params.column.colDef.suppressInactiveSuffix;
        }
            ...
            for (let option of this.options) {
                if (option[this.optionsValueField] && option[this.optionsValueField] === this.value) {
                    if (option[this.optionsDisplayField]) {
                        ...
                        this.showInactiveSuffix = (!this.suppressInactiveSuffix) && ((!option.isActive) || option.isActive === "N");
                        foundOption = true;
                    } else {
